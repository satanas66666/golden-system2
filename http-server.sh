#!/usr/bin/env bash
# Golden System PRO - servidor HTTP de llaves (REV25 fast-bundle + stream-fix + self-healing + legacy)
...
handle_request() {
    ...
    # REV25: consumir todos los encabezados HTTP antes de escribir la respuesta.
    ...
    if [[ "$path" == "__golden_version" ]]; then
        http_reply 200 "OK" "REV25" "$method"
        return 0
    fi
    ...
    # REV25: ruta rápida opcional. El bundle solo existe para keys nuevas creadas
    # por REV25. Si no existe, el cliente vuelve automáticamente al flujo 43/43.
...
listen_server() {
    ...
    # REV25: Python 3 es el listener principal. Lee la petición completa antes
    ...
    # Fallback para instalaciones antiguas sin Python 3. El handler REV25
...
check_install() {
    ...
    if (( rc == 0 )); then
        echo "OK: Golden HTTP REV25 listo en puerto $PORT"
    fi
    return "$rc"
}

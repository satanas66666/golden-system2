#!/usr/bin/env bash
# Golden System PRO - servidor HTTP de llaves (REV18 fast-bundle + stream-fix + self-healing + legacy)
...
handle_request() {
    ...
    if [[ "$path" == "__golden_version" ]]; then
        http_reply 200 "OK" "REV18" "$method"
        return 0
    fi
    ...
    # REV18: ruta rápida opcional. El bundle solo existe para keys nuevas creadas
    # por REV18. Si no existe, el cliente vuelve automáticamente al flujo 43/43.
...
check_install() {
    ...
    if (( rc == 0 )); then
        echo "OK: Golden HTTP REV18 listo en puerto $PORT"
    fi
    return "$rc"
}

#!/usr/bin/env bash
# Golden System PRO - servidor HTTP de llaves (REV17 fast-bundle + stream-fix + self-healing + legacy)
...
handle_request() {
    ...
    if [[ "$path" == "__golden_version" ]]; then
        http_reply 200 "OK" "REV17" "$method"
        return 0
    fi
    ...
    # REV17: ruta rápida opcional. El bundle solo existe para keys nuevas creadas
    # por REV17. Si no existe, el cliente vuelve automáticamente al flujo 43/43.
...
check_install() {
    ...
    if (( rc == 0 )); then
        echo "OK: Golden HTTP REV17 listo en puerto $PORT"
    fi
    return "$rc"
}

#!/usr/bin/env bash
# Golden System PRO - servidor HTTP de llaves (REV15 stream-fix + self-healing + legacy + descarga directa)
...
handle_request() {
    ...
    if [[ "$path" == "__golden_version" ]]; then
        http_reply 200 "OK" "REV15" "$method"
        return 0
    fi
...
check_install() {
    ...
    if (( rc == 0 )); then
        echo "OK: Golden HTTP REV15 listo en puerto $PORT"
    fi
    return "$rc"
}

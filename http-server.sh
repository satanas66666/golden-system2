#!/usr/bin/env bash
# Golden System PRO - servidor HTTP de llaves (REV11 stream-fix + self-healing + legacy + descarga directa)
...
http_file_reply() {
    ...
    printf 'HTTP/1.1 %s %s\r\n' "$code" "$reason"
    printf 'Date: %s\r\n' "$(LC_ALL=C date -R 2>/dev/null || date)"
    printf 'Server: GoldenShellHTTP/2.1\r\n'
...
handle_request() {
    ...
    if [[ "$path" == "__golden_version" ]]; then
        http_reply 200 "OK" "REV11" "$method"
        return 0
    fi

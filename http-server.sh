#!/usr/bin/env bash
# Golden System PRO - servidor HTTP de llaves (REV26 fast-bundle + stream-fix + self-healing + legacy)
...
handle_request() {
    ...
    # REV26: consumir todos los encabezados HTTP antes de escribir la respuesta.
    ...
    if [[ "$path" == "__golden_version" ]]; then
        http_reply 200 "OK" "REV26" "$method"
        return 0
    fi
...
listen_server() {
    ...
    # REV26: Python 3 es el listener principal. Lee la petición completa antes
    ...
    # Fallback para instalaciones antiguas sin Python 3. El handler REV26
...
check_install() {
    ...
    if (( rc == 0 )); then
        echo "OK: Golden HTTP REV26 listo en puerto $PORT"
    fi
    return "$rc"
}

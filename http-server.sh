#!/usr/bin/env bash
# Golden System PRO - servidor HTTP de llaves (REV5 self-healing + legacy compatible)
...
handle_request() {
    ...
    # Endpoint local/simple de salud. Permite detectar el caso en que el proceso
    # existe pero el listener quedó trabado y no atiende keys.
    ...
    if [[ "$arq" != "$INSTALL_LIST" && "$arq" != "$TOOL_LIST" ]]; then
        http_reply 200 "OK" "KEY INVALIDA!" "$method"
        log_msg WARN "lista no permitida key=$key file=$arq peer=$(peer_ip)"
        return 0
    fi
    ...
    if [[ ! -d "$key_dir" || ! -f "$file" || -L "$file" ]]; then
        http_reply 200 "OK" "KEY INVALIDA!" "$method"
        log_msg WARN "key/archivo inexistente key=$key file=$arq peer=$usr_ip"
        return 0
    fi
    ...
    # Prepara los archivos ANTES de devolver la lista; elimina la carrera del servidor anterior.
...
listen_server() {
    ...
    if command -v socat >/dev/null 2>&1; then
        # socat entrega la IP remota mediante SOCAT_PEERADDR.
...
check_install() {
    ...
    if (( rc == 0 )); then
        echo "OK: Golden HTTP listo en puerto $PORT"
    fi
    return "$rc"
}

#!/usr/bin/env bash
# GOLDEN ADM PRO - LuciferMX2019 REV12 / binarios seguros / legacy robusto / progreso visible
cd $HOME
...
payload_fetch() {
    local url="$1" dest="$2" current="$3" total="$4" name="$5" route="$6"
    local old_timeout="$NET_TIMEOUT" old_tries="$NET_TRIES" rc
    NET_TIMEOUT="$PAYLOAD_TIMEOUT"
    NET_TRIES="$PAYLOAD_TRIES"
    if fetch_with_activity "$url" "$dest" "$current" "$total" "$name" "$route"; then
        rc=0
    else
        rc=$?
    fi
...
download_payload_file() {
    ...
    if [[ "${SERVER_REV:-}" == "REV11" || "${SERVER_REV:-}" == "REV12" ]]; then
        rm -f -- "$dest"
        if payload_fetch "$direct" "$dest" "$((current-1))" "$total" "$name" '8888'; then
    ...
    for attempt in 1 2 3; do
        ...
        rm -f -- "$dest"
        if payload_fetch "$legacy" "$dest" "$((current-1))" "$total" "$name" "81 intento ${attempt}/3"; then
            if ! server_error_file "$dest"; then
    ...
    rm -f -- "$dest"
    if payload_fetch "$direct" "$dest" "$((current-1))" "$total" "$name" '8888 final'; then
        if ! server_error_file "$dest"; then
            progress_line "$current" "$total" "$name" 'OK 8888'
            printf '\n'
            return 0

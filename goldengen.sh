...
wait_for_apt() {
    ...
    while pgrep -x apt >/dev/null 2>&1 ||
        ...
        if (( waited == 0 )); then
            info "Esperando a que termine otro proceso APT/DPKG..."
        fi
        sleep 3
        waited=$((waited + 3))
        (( waited < 300 )) || die "APT/DPKG sigue ocupado después de 5 minutos."
    done
}
...
apt_update() {
    wait_for_apt
    if apt-get -o Acquire::Retries=3 update >>"$LOG_FILE" 2>&1; then
        return 0
    fi
    ...
    wait_for_apt
    apt-get -o Acquire::Retries=3 -o Acquire::Check-Valid-Until=false update >>"$LOG_FILE" 2>&1 ||

#!/usr/bin/env bash
# REV23: BASE REV23 ESTABLE + FAST BUNDLE OPCIONAL + UI MODERNA
...
prepare_artifacts() {
    ...
    # REV23: el instalador no continúa con un catálogo incompleto o mezclado.
    ...
    [[ "$(tr -d '\r\n ' <"$TMP_DIR/extract/golden/.golden_revision" 2>/dev/null || true)" == "REV23" ]] ||
        die "golden.zip no corresponde a REV23; evita mezclar revisiones."
}
...
self_test() {
    ...
    [[ "$server_rev" == "REV23" ]] || { warn "Servidor esperado REV23, detectado: ${server_rev:-sin-respuesta}"; rc=1; }

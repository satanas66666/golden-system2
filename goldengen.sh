#!/usr/bin/env bash
# REV25: BASE REV25 ESTABLE + FAST BUNDLE OPCIONAL + UI MODERNA
...
prepare_artifacts() {
    ...
    # REV25: el instalador no continúa con un catálogo incompleto o mezclado.
    ...
    [[ "$(tr -d '\r\n ' <"$TMP_DIR/extract/golden/.golden_revision" 2>/dev/null || true)" == "REV25" ]] ||
        die "golden.zip no corresponde a REV25; evita mezclar revisiones."
}
...
# REV25: el Apache del generador no debe reservar TCP 80. La entrega de
...
for p in files:
    ...
    for line in text.splitlines(True):
        ...
        if m or m2:
            out.append('# Golden REV25: movido a TCP 81 | '+body+nl); changed=True; continue
...
self_test() {
    ...
    [[ "$server_rev" == "REV25" ]] || { warn "Servidor esperado REV25, detectado: ${server_rev:-sin-respuesta}"; rc=1; }

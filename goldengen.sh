#!/usr/bin/env bash
# REV15: APT LOCK FIX + TRANSFERENCIA VALIDADA 43/43 + REPO UNIFICADO
...
apt_lock_pids() {
    ...
    if command -v fuser >/dev/null 2>&1; then
        ...
        # Fallback para imágenes mínimas: ignora zombies. Se usa solamente
        # cuando todavía no existen fuser/lsof.
        pids=$(ps -eo pid=,stat=,comm= 2>/dev/null | awk '
            $2 !~ /^Z/ && ($3=="apt" || $3=="apt-get" || $3=="dpkg" ||
                           $3=="unattended-upgr") {print $1}' || true)
    fi
...
apt_update() {
    wait_for_apt
    if apt-get -o DPkg::Lock::Timeout=600 -o Acquire::Retries=3 update >>"$LOG_FILE" 2>&1; then
        return 0
    fi
    ...
    wait_for_apt
    apt-get -o DPkg::Lock::Timeout=600 -o Acquire::Retries=3 -o Acquire::Check-Valid-Until=false update >>"$LOG_FILE" 2>&1 ||
...
configure_apache() {
    ...
    grep -Eq "^[[:space:]]*Listen[[:space:]]+([^[:space:]]+:)?${PORT_FILES}([[:space:]]|$)" "$ports_conf" ||
        printf '\n# Golden System PRO\nListen %s\n' "$PORT_FILES" >>"$ports_conf"
...
self_test() {
    ...
    [[ "$server_rev" == "REV15" ]] || { warn "Servidor esperado REV15, detectado: ${server_rev:-sin-respuesta}"; rc=1; }

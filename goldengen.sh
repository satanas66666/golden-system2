#!/usr/bin/env bash
# REV21: BASE REV21 ESTABLE + FAST BUNDLE OPCIONAL + UI MODERNA
...
prepare_artifacts() {
    ...
    # REV21: el instalador no continúa con un catálogo incompleto o mezclado.
    ...
    [[ "$(tr -d '\r\n ' <"$TMP_DIR/extract/golden/.golden_revision" 2>/dev/null || true)" == "REV21" ]] ||
        die "golden.zip no corresponde a REV21; evita mezclar revisiones."
}
...
self_test() {
    ...
    [[ "$server_rev" == "REV21" ]] || { warn "Servidor esperado REV21, detectado: ${server_rev:-sin-respuesta}"; rc=1; }
